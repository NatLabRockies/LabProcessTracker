...
class ProcessTrackerGUI(tk.Tk):
    ...
    def create_widgets(self):
        # Operator Name Entry
        tk.Label(self, text="Operator Name:", bg="#f0f0f0").pack(pady=(10,0))
        ...
        self.save_btn = tk.Button(btn_frame, text="SAVE", width=10, command=self.save_log)
        self.save_btn.grid(row=0, column=0, padx=5)
        self.undo_btn = tk.Button(btn_frame, text="UNDO", width=10, command=self.undo_last_scan)
        self.undo_btn.grid(row=0, column=1, padx=5)
        self.exit_btn = tk.Button(btn_frame, text="EXIT", width=10, command=self.exit_app)
    ...
    def handle_scan(self):
        ...
        if not data_type:
            self.print_terminal(f"[ERROR] Invalid format: '{qr_text}'. Use 'TYPE{DATA_SEPARATOR}ID' (e.g., PROCESS:Name or SAMPLE:ID).")
            return
        if data_type == 'PROCESS':
            self.current_process = data_id
            self.update_process_block(data_id)
            self.update_sample_block(None, status_type="RESET")
            self.print_terminal(f">>> PROCESS UPDATED: Now running: '{self.current_process}'")
        elif data_type == 'SAMPLE':
            if self.current_process:
                self.log_scan_event(self.current_process, data_id)
                self.update_sample_block(data_id, status_type="SAMPLE")
            else:
                self.print_terminal("[ALERT] Cannot log sample. Please scan a **PROCESS QR code** first to define the current step.")
                self.update_sample_block("No process set", status_type="ALERT")
        else:
            self.print_terminal(f"[ERROR] Unknown data type scanned: '{data_type}'. Must be 'PROCESS' or 'SAMPLE'.")
